...
function UserCard({ user, expanded, onToggle, onEdit, onDelete }: {
  ...
  return (
    <div className={cn('bg-card border rounded-2xl overflow-hidden transition-all shadow-sm', isAdmin ? 'border-primary/30' : 'border-border')}>
      <div
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="w-8 h-8 rounded-lg bg-muted/50 flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all">
            <Edit3 size={14} />
          </button>
          {!isAdmin && (
            <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="w-8 h-8 rounded-lg bg-muted/50 flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all">
              <Trash2 size={14} />
            </button>
          )}
      ...
      {expanded && (
        <div className="px-3.5 pb-3.5 border-t border-border/50 pt-3 grid grid-cols-2 gap-2 text-xs animate-in fade-in slide-in-from-top-2 duration-300">
...
function UserForm({ user, onClose, onSave }: {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-end justify-center animate-in fade-in duration-300" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-t-[2.5rem] w-full max-w-lg p-6 pb-24 space-y-6 max-h-[92vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom duration-500"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between sticky top-0 bg-card/80 backdrop-blur-md z-10 py-2">
        ...
        <section className="space-y-4">
            ...
            <>
              ...
              <Field label="كلمة المرور *">
                <div className="relative">
                  <input type={showPass ? 'text' : 'password'} value={form.password || ''} onChange={e => set('password', e.target.value)} className={cn(inputCls, 'pe-12')} placeholder="كلمة المرور" dir="ltr" />
          ...
          {user && (
            <Field label="كلمة المرور الجديدة (اتركها فارغة لعدم التغيير)">
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} value={form.password || ''} onChange={e => set('password', e.target.value)} className={cn(inputCls, 'pe-12')} placeholder="اترك فارغاً للإبقاء" dir="ltr" />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground">
                  {showPass ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
